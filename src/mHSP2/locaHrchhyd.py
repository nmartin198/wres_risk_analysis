...
If true, yse the original auxil function to calculate surface area
...
def createTSIVOL( targID, iI ):
    ...
    from locaCoupling import getGWIVOLbyTargTS, getGWIOVOLbyTargTS
    # globals
    ...
    mf6ivol = float( 0.0 )  # inflow volume from MODFLOW 6
    pero = float ( 0.0 )    # pervious outflow
    suro = float ( 0.0 )    # impervious outflow
    ovol = float ( 0.0 )    # rchres outflow
    # start
    # external is in acre-ft/day convert to ft3/day
    ivol = float( EXIVOL[targID][iI] ) * VFACT
    mf6ivol = getGWIVOLbyTargTS( iI, targID ) * VFACT
    ivol = ivol + mf6ivol
    ...
    for uL in usLinks:
        ...
        if sType == TARG_PERVLND:
            if sOut == "PERO":
                ...
                gwOvolAF = getGWIOVOLbyTargTS( iI, sID )
                gwOVol = gwOvolAF * VFACT
                ivol = ivol + peroVol + gwOVol
            else:
...
def demand(vol, rowFT, funct, nexits, delts, convf, colind, outdgt, ODGTF):
    ...
    Args:
        ...
        colind (np.array): Array for MAX_EXITS that has the multiplier and 
            FTAB column value to use for each exit
