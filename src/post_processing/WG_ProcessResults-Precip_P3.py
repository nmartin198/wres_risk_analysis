...
@author: nnmartin <nick.martin@stanfordalumni.org>
...
if __name__ == "__main__":
    ...
    for jJ in range( PRE_START_IND, TotNum, 1):
        ...
        for iI in range(1, NUM_REAL + 1, 1):
            ...
            H0PreProj3 = H0DF.loc[ ProjStart3:ProjEnd3, OutColsList[PRE_START_IND:] ].copy()
            # now split out our realization values
            if iI == 1:
                # H0 proj 3
                curCol = OutColsList[jJ]
                H0P3PbyGrid = H0PreProj3[[curCol]].copy()
                H0P3PbyGrid.columns = [ curReal ]
                H0PreP3DF = H0P3PbyGrid
            else:
                # H0 proj 3
                curCol = OutColsList[jJ]
                tH0P3PbyGrid = H0PreProj3[[curCol]].copy()
                tH0P3PbyGrid.columns = [ curReal ]
                H0PreP3DF = H0PreP3DF.merge( tH0P3PbyGrid, how='inner',
                                                       left_index=True,
                                                       right_index=True )
            # then H1 case
            #    statistics for H1 case should be the same for each period
            H1PreProj3 = H1DF.loc[ ProjStart3:ProjEnd3, OutColsList[PRE_START_IND:] ].copy()
            # now split out our realization values
            if iI == 1:
                # H0 proj 3
                curCol = OutColsList[jJ]
                H1P3PbyGrid = H1PreProj3[[curCol]].copy()
                H1P3PbyGrid.columns = [ curReal ]
                H1PreP3DF = H1P3PbyGrid
            else:
                # H0 proj 3
                curCol = OutColsList[jJ]
                tH1P3PbyGrid = H1PreProj3[[curCol]].copy()
                tH1P3PbyGrid.columns = [ curReal ]
                H1PreP3DF = H1PreP3DF.merge( tH1P3PbyGrid, how='inner',
                                                       left_index=True,
                                                       right_index=True )
            # end if
        ...
        cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                'Processed', 'H0P3PrecipAllReal_G%d.pickle' % cGID ) )
        H0PreP3DF.to_pickle( cOutFP, compression='zip' )
        cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                'Processed', 'H1P3PrecipAllReal_G%d.pickle' % cGID ) )
        H1PreP3DF.to_pickle( cOutFP, compression='zip' )
    # end of grid ID for
    # end
