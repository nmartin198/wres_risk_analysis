...
if __name__ == "__main__":
    ...
    for iI in range(1, NUM_REAL + 1, 1):
        ...
        H0TmpProj2 = H0DF.loc[ ProjStart2:ProjEnd2, OutColsList[:PRE_START_IND] ].copy()
        # now split out our realization values
        if iI == 1:
            # H0 proj 2
            H0P2TmaxAR = H0TmpProj2[[OutColsList[0]]].copy()
            H0P2TmaxAR.columns = [curReal]
            H0P2TminAR = H0TmpProj2[[OutColsList[1]]].copy()
            H0P2TminAR.columns = [curReal]
        else:
            # H0 proj 2
            tH0P2TmaxAR = H0TmpProj2[[OutColsList[0]]].copy()
            tH0P2TmaxAR.columns = [curReal]
            H0P2TmaxAR = H0P2TmaxAR.merge( tH0P2TmaxAR, how='inner', 
                                           left_index=True, right_index=True )
            tH0P2TminAR = H0TmpProj2[[OutColsList[1]]].copy()
            tH0P2TminAR.columns = [curReal]
            H0P2TminAR = H0P2TminAR.merge( tH0P2TminAR, how='inner', 
                                           left_index=True, right_index=True )
        # then H1 case
        #    statistics for H1 case should be the same for each period
        H1TmpProj2 = H1DF.loc[ ProjStart2:ProjEnd2, OutColsList[:PRE_START_IND] ].copy()
        # now split out our realization values
        if iI == 1:
            # H0 proj 2
            H1P2TmaxAR = H1TmpProj2[[OutColsList[0]]].copy()
            H1P2TmaxAR.columns = [curReal]
            H1P2TminAR = H1TmpProj2[[OutColsList[1]]].copy()
            H1P2TminAR.columns = [curReal]
        else:
            # H0 proj 2
            tH1P2TmaxAR = H1TmpProj2[[OutColsList[0]]].copy()
            tH1P2TmaxAR.columns = [curReal]
            H1P2TmaxAR = H1P2TmaxAR.merge( tH1P2TmaxAR, how='inner', 
                                           left_index=True, right_index=True )
            tH1P2TminAR = H1TmpProj2[[OutColsList[1]]].copy()
            tH1P2TminAR.columns = [curReal]
            H1P2TminAR = H1P2TminAR.merge( tH1P2TminAR, how='inner', 
                                           left_index=True, right_index=True )
    # end of realization for
    # outputs
    # H0 by category
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H0P2TmaxAllReal_P2.pickle' ) )
    H0P2TmaxAR.to_pickle( cOutFP, compression='zip' )
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H0P2TminAllReal_P2.pickle' ) )
    H0P2TminAR.to_pickle( cOutFP, compression='zip' )
    # H1 by category
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H1P2TmaxAllReal_P2.pickle' ) )
    H1P2TmaxAR.to_pickle( cOutFP, compression='zip' )
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H1P2TminAllReal_P2.pickle' ) )
    H1P2TminAR.to_pickle( cOutFP, compression='zip' )
