...
if __name__ == "__main__":
    ...
    ProjStart3 = WGI.PROJ_PERIODS[2][0]
    ProjEnd3 = WGI.PROJ_PERIODS[2][1]
    ...
    for iI in range(1, NUM_REAL + 1, 1):
        ...
        H0TmpProj3 = H0DF.loc[ ProjStart3:ProjEnd3, OutColsList[:PRE_START_IND] ].copy()
        # now split out our realization values
        if iI == 1:
            # H0 proj 3
            H0P3TmaxAR = H0TmpProj3[[OutColsList[0]]].copy()
            H0P3TmaxAR.columns = [curReal]
            H0P3TminAR = H0TmpProj3[[OutColsList[1]]].copy()
            H0P3TminAR.columns = [curReal]
        else:
            # H0 proj 3
            tH0P3TmaxAR = H0TmpProj3[[OutColsList[0]]].copy()
            tH0P3TmaxAR.columns = [curReal]
            H0P3TmaxAR = H0P3TmaxAR.merge( tH0P3TmaxAR, how='inner', 
                                           left_index=True, right_index=True )
            tH0P3TminAR = H0TmpProj3[[OutColsList[1]]].copy()
            tH0P3TminAR.columns = [curReal]
            H0P3TminAR = H0P3TminAR.merge( tH0P3TminAR, how='inner', 
                                           left_index=True, right_index=True )
        # then H1 case
        #    statistics for H1 case should be the same for each period
        H1TmpProj3 = H1DF.loc[ ProjStart3:ProjEnd3, OutColsList[:PRE_START_IND] ].copy()
        # now split out our realization values
        if iI == 1:
            # H0 proj 3
            H1P3TmaxAR = H1TmpProj3[[OutColsList[0]]].copy()
            H1P3TmaxAR.columns = [curReal]
            H1P3TminAR = H1TmpProj3[[OutColsList[1]]].copy()
            H1P3TminAR.columns = [curReal]
        else:
            # H0 proj 3
            tH1P3TmaxAR = H1TmpProj3[[OutColsList[0]]].copy()
            tH1P3TmaxAR.columns = [curReal]
            H1P3TmaxAR = H1P3TmaxAR.merge( tH1P3TmaxAR, how='inner', 
                                           left_index=True, right_index=True )
            tH1P3TminAR = H1TmpProj3[[OutColsList[1]]].copy()
            tH1P3TminAR.columns = [curReal]
            H1P3TminAR = H1P3TminAR.merge( tH1P3TminAR, how='inner', 
                                           left_index=True, right_index=True )
    # end of realization for
    # outputs
    # H0 by category
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H0P3TmaxAllReal_P3.pickle' ) )
    H0P3TmaxAR.to_pickle( cOutFP, compression='zip' )
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H0P3TminAllReal_P3.pickle' ) )
    H0P3TminAR.to_pickle( cOutFP, compression='zip' )
    # H1 by category
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H1P3TmaxAllReal_P3.pickle' ) )
    H1P3TmaxAR.to_pickle( cOutFP, compression='zip' )
    cOutFP = path.normpath( path.join( WGI.OUT_DIR, WGI.OUT_SUB_DIR, 
                                       'Processed', 'H1P3TminAllReal_P3.pickle' ) )
    H1P3TminAR.to_pickle( cOutFP, compression='zip' )
