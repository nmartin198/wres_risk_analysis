...
if __name__ == '__main__':
    ...
    for yY in AllYrs:
        ...
        for gG in range( 1, (NumGrid + 1), 1):
            ...
            TMxWRSum[:LastDay] = ( TMxWRSum[:LastDay] + ( WetState *
                                   np.array( YrNodeDF[DBAD.FIELDN_TMXVAL],
                                             dtype=np.float32 ) ) )
            TAvWRSum[:LastDay] = ( TAvWRSum[:LastDay] + ( WetState *
                                   np.array( YrNodeDF[DBAD.FIELDN_TAVEVAL],
                                             dtype=np.float32 ) ) )
            TMnWRSum[:LastDay] = ( TMnWRSum[:LastDay] + ( WetState *
                                   np.array( YrNodeDF[DBAD.FIELDN_TMNVAL],
                                             dtype=np.float32 ) ) )
            DpWRSum[:LastDay] = ( DpWRSum[:LastDay] + ( WetState *
                                   np.array( YrNodeDF[DBAD.FIELDN_TDPT],
                                             dtype=np.float32 ) ) )
            RHWRSum[:LastDay] = ( RHWRSum[:LastDay] + ( WetState *
                                   np.array( YrNodeDF[RELHUM],
                                             dtype=np.float32 ) ) )
    ...
    WDDict[DBAD.FIELDN_TMXVAL] = tWAve
    DDDict[DBAD.FIELDN_TMXVAL] = tDAve
    # ave temp
    tWAve = wMulti * TAvWRSum
    tDAve = dMulti * TAvDRSum
    WDDict[DBAD.FIELDN_TAVEVAL] = tWAve
    DDDict[DBAD.FIELDN_TAVEVAL] = tDAve
    # min temp
    tWAve = wMulti * TMnWRSum
    tDAve = dMulti * TMnDRSum
    WDDict[DBAD.FIELDN_TMNVAL] = tWAve
    DDDict[DBAD.FIELDN_TMNVAL] = tDAve
    # dew point
    tWAve = wMulti * DpWRSum
    tDAve = dMulti * DpDRSum
    WDDict[DBAD.FIELDN_TDPT] = tWAve
    DDDict[DBAD.FIELDN_TDPT] = tDAve
    # relative humidity
    ...
    # output to Excel
    OFNameRoot = "OWeather_Aves_%s-%s" % (StartYear, EndYear)
