...
LogFile = "CMIP5OWExAve_%s.txt" % DS_DESC
...
if __name__ == '__main__':
    ...
    for gG in range(GD_START, (GD_END + 1)):
        ...
        for yY in AllYrs4:
            ...
            if (NPrecipDays < 1) or (NTMaxDays < 1) or (NTMinDays < 1):
                ...
                with open( LogFile, 'a' ) as LID:
                    LID.write("Returned empty arrays for year %d" % yY)
                continue
            if (NPrecipDays != NTMaxDays) or (NTMaxDays != NTMinDays):
                print("Length of annual precip, tmax, and tmin data sets are " \
                      "different for year %d.\nPrecip = %d, TMax = %d, " \
    ...
    WDDict1[DBAD.FIELDN_TMXVAL] = tWAve1
    WDDict2[DBAD.FIELDN_TMXVAL] = tWAve2
    WDDict3[DBAD.FIELDN_TMXVAL] = tWAve3
    WDDict4[DBAD.FIELDN_TMXVAL] = tWAve4
    DDDict1[DBAD.FIELDN_TMXVAL] = tDAve1
    DDDict2[DBAD.FIELDN_TMXVAL] = tDAve2
    DDDict3[DBAD.FIELDN_TMXVAL] = tDAve3
    DDDict4[DBAD.FIELDN_TMXVAL] = tDAve4
    # min temp
    ...
    WDDict1[DBAD.FIELDN_TMNVAL] = tWAve1
    WDDict2[DBAD.FIELDN_TMNVAL] = tWAve2
    WDDict3[DBAD.FIELDN_TMNVAL] = tWAve3
    WDDict4[DBAD.FIELDN_TMNVAL] = tWAve4
    DDDict1[DBAD.FIELDN_TMNVAL] = tDAve1
    DDDict2[DBAD.FIELDN_TMNVAL] = tDAve2
    DDDict3[DBAD.FIELDN_TMNVAL] = tDAve3
    DDDict4[DBAD.FIELDN_TMNVAL] = tDAve4
    ...
    # output to Excel
    OFNameRoot1 = "OWeather_%s_Aves_%s-%s" % (DS_DESC, StartYear1, EndYear1)
    OFNameRoot2 = "OWeather_%s_Aves_%s-%s" % (DS_DESC, StartYear2, EndYear2)
    OFNameRoot3 = "OWeather_%s_Aves_%s-%s" % (DS_DESC, StartYear3, EndYear3)
    OFNameRoot4 = "OWeather_%s_Aves_%s-%s" % (DS_DESC, StartYear4, EndYear4)
    ...
    OFNameRoot1 = "OWeathWetDF_%s_Ave_%s-%s" % (DS_DESC, StartYear1, EndYear1)
    OFNameRoot2 = "OWeathWetDF_%s_Ave_%s-%s" % (DS_DESC, StartYear2, EndYear2)
    OFNameRoot3 = "OWeathWetDF_%s_Ave_%s-%s" % (DS_DESC, StartYear3, EndYear3)
    OFNameRoot4 = "OWeathWetDF_%s_Ave_%s-%s" % (DS_DESC, StartYear4, EndYear4)
    ...
    OFNameRoot1 = "OWeathDryDF_%s_Ave_%s-%s" % (DS_DESC, StartYear1, EndYear1)
    OFNameRoot2 = "OWeathDryDF_%s_Ave_%s-%s" % (DS_DESC, StartYear2, EndYear2)
    OFNameRoot3 = "OWeathDryDF_%s_Ave_%s-%s" % (DS_DESC, StartYear3, EndYear3)
    OFNameRoot4 = "OWeathDryDF_%s_Ave_%s-%s" % (DS_DESC, StartYear4, EndYear4)
    OFName1 = "%s.pickle" % OFNameRoot1
    OFName2 = "%s.pickle" % OFNameRoot2
    OFName3 = "%s.pickle" % OFNameRoot3
    OFName4 = "%s.pickle" % OFNameRoot4
    ...
    CurDT = dt.datetime.now()
    with open( LogFile, 'a' ) as LID:
        LID.write("Finish processing CMIP5 data at %s\n\n" %
                  CurDT.strftime("%m/%d/%Y %H:%M:%S") )
    # end of with block
    # end of work
